...
indices_to_modify = [(0, 0, 0), (1, 0, 0)]
for idx in indices_to_modify:
    max_values_re[idx] -= rt_output_re[idx]

max_values_2d = np.squeeze(max_values_re)
max_values_trans = np.empty((11, 3))

max_values_trans[:4] = max_values_2d[7:]
max_values_trans[4:11] = max_values_2d[:7]

# 保存CSV文件至工作路径
file_name = "max_values.csv"
with open(file_name, mode='w') as file:
    writer = csv.writer(file)
    writer.writerows(max_values_trans)
